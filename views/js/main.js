...
var resizePizzas = function(size) {
  ...
  // --- Moved variables outside of loop.  dx and newwidth only need to calc once
  // --- used the more efficient getElementsByClassName instead of queryselectorALl

  function changePizzaSizes(size) {

    var PizzaClass = document.getElementsByClassName("randomPizzaContainer");
    var dx = determineDx(PizzaClass[0], size);
    var newwidth = PizzaClass[0].offsetWidth + dx + 'px';

    for (var i = 0; i < PizzaClass.length; i++) {
      PizzaClass[i].style.width = newwidth;
    }
  }
...
// --- Moved document.body.scrollTop out of the loop so the DOM is not accessed each time.
// --- removed the i%5 since the number won't change due to how math works.
...
// ------ Reduced the for loop to only render 35 pizzas instead of 200, works for monitors up to 27"
